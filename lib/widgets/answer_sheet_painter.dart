...
class AnswerSheetPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final int questionsPerRegion =
        (template.totalQuestions / template.answerRegions.length).ceil();
    ...
    for (int i = 0; i < template.answerRegions.length; i++) {
      ...
      for (int q = 0; q < questionsInThisRegion; q++) {
        ...
        textPainter.paint(
            canvas,
            Offset(left - textPainter.width - 8,
                rowY + (rowHeight / 2) - (textPainter.height / 2)));

        // Choice Bubbles (Inside)
        final double bubbleAreaWidth =
            totalWidth - (template.innerPadding * size.width * 2);
        final double bubbleSpacing =
            bubbleAreaWidth / (template.choicesPerQuestion + 1);
