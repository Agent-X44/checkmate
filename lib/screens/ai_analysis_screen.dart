...
class _AIAnalysisScreenState extends State<AIAnalysisScreen> {
  ...
  double _stripHeightMultiplier = Standard50QuestionsTemplate.defaultStripHeight;
  ...
  void initState() {
    ...
    _customSet = defaultTemplate.setRegion ?? const Rect.fromLTRB(0.18, 0.15, 0.35, 0.22);
  ...
  Future<void> _startBackgroundProcessing() async {
    _isolate = await Isolate.spawn(ImageProcessor.edgeDetectionWorker, _receivePort.sendPort);
    _receivePort.listen((message) {
      if (message is SendPort) {
        message.send(OmrRequest(
          bytes: widget.rawImage!, corners: widget.rawCorners!,
          template: Standard50QuestionsTemplate(), replyPort: _receivePort.sendPort,
        ...
        setState(() { _currentSheet = message; _phase = 1; });
      }
    });
  }
  ...
  void _reprocessResults() {
    ...
    try {
      final warpedMat = cv.imdecode(_currentSheet!.warpedImage, cv.IMREAD_COLOR);
      ...
      if (_setBubbles.isNotEmpty) {
        ...
        final res = BubbleDetectionService.detectFilledBubble(
          setMat, 2, isBinary: true, 
          gridStart: _setGridStart, gridWidthRatio: _setGridWidth,
          zoneWidthRatio: _setZoneWidth, zoneHeightRatio: _setZoneHeight,
      ...
      for (int i = 0; i < 2; i++) {
        final regionMat = TemplateService.extractRegion(thresholded, regions[i]);
        final count = (i == 1) ? template.totalQuestions - qPerRegion : qPerRegion;
      ...
      for (var p in _customBubbles) {
        final int x = ((p.dx + (_xOffset / thresholded.width)) * thresholded.width).toInt();
        final int y = ((p.dy + (_yOffset / thresholded.height)) * thresholded.height).toInt();
        ...
        final double fillRatio = cv.countNonZero(bubbleMat) / (areaSize * areaSize);
      ...
      warpedMat.dispose(); thresholded.dispose();
    } catch (_) {}
  }
                ...
                Navigator.push(
                  ...
                  MaterialPageRoute(
                    builder: (context) => TemplateDesignerScreen(
                      ...
                      onApply: (newBubbles, newBoxes, newQr, newSet, newSetBubbles) {
                        setState(() {
        ...
        child: Container(
          ...
          child: Column(
            children: [
              ...
              Expanded(child: AnimatedSwitcher(duration: const Duration(milliseconds: 400), child: _phase == 0 ? _buildProcessingUI() : _buildResultsUI())),
            ],
          ),
        ),
      ),
  ...
  Widget _buildDebugSliders() {
    return Container(
      ...
      child: Column(
        ...
        children: [
          const Text("QUICK ALIGNMENT", style: TextStyle(color: Colors.yellowAccent, fontWeight: FontWeight.bold, fontSize: 12)),
          SizedBox(
            height: 220,
            child: ListView(
              children: [
                _debugSlider("Col1 X", _col1.left, 0.0, 1.0, (v) { setState(() => _col1 = Rect.fromLTWH(v, _col1.top, _col1.width, _col1.height)); _reprocessResults(); }),
                _debugSlider("Col1 Y", _col1.top, 0.0, 1.0, (v) { setState(() => _col1 = Rect.fromLTWH(_col1.left, v, _col1.width, _col1.height)); _reprocessResults(); }),
                _debugSlider("Col2 X", _col2.left, 0.0, 1.0, (v) { setState(() => _col2 = Rect.fromLTWH(v, _col2.top, _col2.width, _col2.height)); _reprocessResults(); }),
                _debugSlider("Col2 Y", _col2.top, 0.0, 1.0, (v) { setState(() => _col2 = Rect.fromLTWH(_col2.left, v, _col2.width, _col2.height)); _reprocessResults(); }),
                _debugSlider("Y-Offset", _yOffset, -50.0, 50.0, (v) { setState(() => _yOffset = v); _reprocessResults(); }),
                const Divider(color: Colors.white24),
                _sectionHeader("EXAM SET ALIGNMENT"),
                _debugSlider("Set X", _customSet?.left ?? 0, 0.0, 1.0, (v) { 
                  setState(() => _customSet = Rect.fromLTWH(v, _customSet?.top ?? 0, _customSet?.width ?? 0.1, _customSet?.height ?? 0.05)); 
                  _reprocessResults(); 
                }),
                _debugSlider("Set Y", _customSet?.top ?? 0, 0.0, 1.0, (v) { 
                  setState(() => _customSet = Rect.fromLTWH(_customSet?.left ?? 0, v, _customSet?.width ?? 0.1, _customSet?.height ?? 0.05)); 
                  _reprocessResults(); 
                }),
                _debugSlider("Set W", _customSet?.width ?? 0, 0.01, 0.5, (v) { 
                  setState(() => _customSet = Rect.fromLTWH(_customSet?.left ?? 0, _customSet?.top ?? 0, v, _customSet?.height ?? 0.05)); 
                  _reprocessResults(); 
                }),
                _debugSlider("Set H", _customSet?.height ?? 0, 0.01, 0.5, (v) { 
                  setState(() => _customSet = Rect.fromLTWH(_customSet?.left ?? 0, _customSet?.top ?? 0, _customSet?.width ?? 0.1, v)); 
                  _reprocessResults(); 
                }),
                _debugSlider("Set G-Start", _setGridStart, 0.0, 0.5, (v) { setState(() => _setGridStart = v); _reprocessResults(); }),
                _debugSlider("Set G-Width", _setGridWidth, 0.5, 1.0, (v) { setState(() => _setGridWidth = v); _reprocessResults(); }),
                _debugSlider("Set G-YPos", _setGridYPos, 0.0, 1.0, (v) { setState(() => _setGridYPos = v); _reprocessResults(); }),
                _debugSlider("Set Z-Width", _setZoneWidth, 0.1, 1.0, (v) { setState(() => _setZoneWidth = v); _reprocessResults(); }),
                _debugSlider("Set Z-Height", _setZoneHeight, 0.1, 1.0, (v) { setState(() => _setZoneHeight = v); _reprocessResults(); }),
                const Divider(color: Colors.white24),
                _sectionHeader("OMR GRID SETTINGS"),
                _debugSlider("Grid Space", _gridStart, 0.0, 0.5, (v) { setState(() => _gridStart = v); _reprocessResults(); }),
                _debugSlider("Grid Size", _gridWidthRatio, 0.5, 1.0, (v) { setState(() => _gridWidthRatio = v); _reprocessResults(); }),
                _debugSlider("Threshold", _bubbleThreshold, 0.05, 0.5, (v) { setState(() => _bubbleThreshold = v); _reprocessResults(); }),
                _debugSlider("Strip Height", _stripHeightMultiplier, 0.5, 2.5, (v) { setState(() => _stripHeightMultiplier = v); _reprocessResults(); }),
                _debugSlider("Grid Y-Space", _gridYSpace, -5.0, 5.0, (v) { setState(() => _gridYSpace = v); _reprocessResults(); }),
                _debugSlider("X-Offset", _xOffset, -50.0, 50.0, (v) { setState(() => _xOffset = v); _reprocessResults(); }),
              ],
            ),
          ),
        ],
  ...
  Widget _sectionHeader(String title) {
    return Padding(
      ...
      child: Text(title, style: const TextStyle(color: Colors.yellowAccent, fontSize: 9, fontWeight: FontWeight.bold)),
    );
  }
  ...
  Widget _debugSlider(String label, double val, double min, double max, ValueChanged<double> onChanged) {
    return Row(
      children: [
        SizedBox(width: 70, child: Text(label, style: const TextStyle(fontSize: 10, color: Colors.white))),
        Expanded(child: Slider(value: val.clamp(min, max), min: min, max: max, onChanged: onChanged, activeColor: Colors.yellowAccent)),
        SizedBox(width: 40, child: Text(val.toStringAsFixed(3), style: const TextStyle(fontSize: 9, color: Colors.yellowAccent))),
      ],
    );
  }
  ...
  Widget _buildProcessingUI() {
    return const Column(mainAxisAlignment: MainAxisAlignment.center, children: [
      CircularProgressIndicator(strokeWidth: 6), SizedBox(height: 24),
      Text('Processing Paper...', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
    ]);
  }
  ...
  Widget _buildResultsUI() {
    ...
    return ListView(
      ...
      children: [
        ...
        const Text('DETECTION RESULTS', style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: Colors.grey)),
        const SizedBox(height: 10),
        if (sheet.detectedSet != null) _buildSetResultTile(sheet.detectedSet!),
        ...List.generate(sheet.results.length, (i) => _buildResultTile(sheet, i)),
        const SizedBox(height: 40),
        ElevatedButton(onPressed: () => Navigator.pop(context), style: ElevatedButton.styleFrom(minimumSize: const Size(double.infinity, 55)), child: const Text('FINISH SESSION')),
      ],
    );
  }
  ...
  Widget _buildStudentInfo(dynamic qr) {
    ...
    return Column(
      children: [
        Card(
          margin: const EdgeInsets.only(bottom: 12), elevation: 0, color: Colors.blueAccent.withValues(alpha: 0.05),
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12), side: BorderSide(color: Colors.blueAccent.withValues(alpha: 0.2))),
          child: Padding(padding: const EdgeInsets.all(16), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(
              mainAxisAlignment: MainAxisAlignment.spaceBetween,
              children: [
                Expanded(child: Text(qr.studentName, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold))),
                if (sheet?.detectedSet != null)
                  Container(
                    padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 4),
                    decoration: BoxDecoration(color: Colors.blueAccent, borderRadius: BorderRadius.circular(8)),
                    child: Text(sheet!.detectedSet!, style: const TextStyle(color: Colors.white, fontSize: 12, fontWeight: FontWeight.bold)),
                  ),
              ],
            ),
            Text("${qr.examTitle} • ${qr.examCode}", style: const TextStyle(color: Colors.grey, fontSize: 12)),
            const Divider(height: 24, color: Colors.blueAccent),
            Row(
              children: [
                const Icon(Icons.qr_code_scanner, size: 16, color: Colors.blueAccent),
                const SizedBox(width: 8),
                const Text("QR CONTENT:", style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.blueAccent)),
                const SizedBox(width: 8),
                Expanded(
                  child: Text(
                    qr.toString(), 
  ...
  Widget _buildSetResultTile(String setLabel) {
    return Card(
      ...
      child: ListTile(
        ...
        title: const Text(
          "EXAM SET DETECTED",
          style: TextStyle(fontSize: 10, fontWeight: FontWeight.bold, color: Colors.blueAccent, letterSpacing: 1.1),
        ),
        subtitle: Text(
          setLabel,
  ...
  void _updateDetectedSet(String newSet) {
    ...
    setState(() {
      _currentSheet = ProcessedSheet(
        ...
        qrData: _currentSheet!.qrData,
        detectedSet: newSet,
        templateName: _currentSheet!.templateName.isNotEmpty ? _currentSheet!.templateName : template.name,
      );
    });
  }
  ...
  Widget _buildResultTile(ProcessedSheet sheet, int i) {
    ...
    return Card(
      ...
      shape: RoundedRectangleBorder(
        borderRadius: BorderRadius.circular(12),
        side: BorderSide(
          color: isAmbiguous ? Colors.orange : Colors.transparent, 
          width: 2
        ),
      ),
      child: ListTile(
        ...
        leading: CircleAvatar(
          ...
          backgroundColor: isAmbiguous 
              ? Colors.orange 
              : (res.answer != null ? Colors.green : Colors.red.withValues(alpha: 0.1)), 
          child: Text("${i+1}", style: const TextStyle(fontSize: 10, color: Colors.white))
        ),
        title: Row(
          children: [
            Text(
              res.answer ?? (isAmbiguous ? "MULTIPLE ANSWERS" : "MISSING"), 
              style: TextStyle(
                fontWeight: FontWeight.bold, 
                color: isAmbiguous ? Colors.orange : (res.answer == null ? Colors.red : null)
              )
            ),
            if (isAmbiguous) ...[
              const SizedBox(width: 8),
        ...
        subtitle: Column(
          ...
          children: [
            ...
            ClipRRect(
              ...
              child: Image.memory(sheet.questionImages[i], height: 40, fit: BoxFit.fitHeight)
            ),
            if (isAmbiguous) 
              Text(
                "Detected: ${res.multipleAnswers.join(', ')}", 
        ...
        trailing: Column(
          ...
          children: [
            Text("${(res.confidence * 100).toInt()}%", style: const TextStyle(fontSize: 10, color: Colors.grey)),
            if (isAmbiguous)
              const Text("TAP TO FIX", style: TextStyle(fontSize: 8, color: Colors.orange, fontWeight: FontWeight.bold)),
          ],
        ),
      ),
    );
    ...
    showDialog(
      ...
      builder: (context) => AlertDialog(
        ...
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            const Text("Multiple answers detected. Please select the intended answer:"),
...
class BoxAlignmentPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    final paint = Paint()..color = Colors.red.withValues(alpha: 0.6)..style = PaintingStyle.stroke..strokeWidth = 2.0;
    final gridPaint = Paint()..color = Colors.yellowAccent.withValues(alpha: 0.3)..style = PaintingStyle.stroke..strokeWidth = 1.0;
    final stripPaint = Paint()..color = Colors.yellowAccent.withValues(alpha: 0.1)..style = PaintingStyle.fill;
    final setPaint = Paint()..color = Colors.greenAccent..style = PaintingStyle.stroke..strokeWidth = 2.0;
    ...
    for (var r in regions) {
      ...
      for (int i = 0; i < qCount; i++) {
        final cY = rect.top + ((i + 0.5) * rowStep) + yOffset + (i * gridYSpace);
        final h = rowStep * stripHeight;
        final t = cY - (h / 2);
        final b = cY + (h / 2);
        if (b > rect.top && t < rect.bottom) {
          final sRect = Rect.fromLTRB(rect.left, math.max(rect.top, t), rect.right, math.min(rect.bottom, b));
    ...
    if (setRegion != null) {
      final r = Rect.fromLTRB(setRegion!.left * size.width, setRegion!.top * size.height, setRegion!.right * size.width, setRegion!.bottom * size.height);
    ...
    for (var p in setBubbles) {
      canvas.drawCircle(Offset(p.dx * size.width, p.dy * size.height), 12, setPaint);
    }
  }
