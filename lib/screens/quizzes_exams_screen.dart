...
class _QuizzesExamsScreenState extends State<QuizzesExamsScreen> {
  ...
  Future<void> _generateSheets(String examId, String examTitle, {bool hasMultipleSets = false, String? templateId}) async {
    try {
      ...
      final mcqCount = questions.where((q) => (q['question_type'] ?? q['questionType']) == 'MCQ').length;
      final tfCount = questions.where((q) => (q['question_type'] ?? q['questionType']) == 'TF').length;
      ...
      template ??= AnswerSheetTemplateRegistry.forConfiguration(totalQuestions > 0 ? totalQuestions : 50, mcqCount, tfCount);
      template ??= AnswerSheetTemplateRegistry.forQuestionCount(totalQuestions > 0 ? totalQuestions : 50);
  ...
  void _showEditQuestionDialog(Map<String, dynamic> q, Function(Map<String, dynamic>) onSave) {
    final textController = TextEditingController(text: q['question_text'] ?? '');
    ...
    if (!isTF && options.length < 4) {
      options = List.from(options)..addAll(List.generate(4 - options.length, (_) => ''));
    }
    ...
    String currentAnswer = (q['correct_answer'] ?? 'A').toString().toUpperCase();
        ...
        return StatefulBuilder(
          builder: (context, setDialogState) {
            ...
            return AlertDialog(
              ...
              title: Text('Edit Question', style: TextStyle(color: textColor, fontWeight: FontWeight.bold)),
              content: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    ...
                    if (!isTF) ...[
                      Text('Options:', style: TextStyle(fontWeight: FontWeight.bold, color: textColor)),
                    ...
                    const SizedBox(height: 16),
                    Text('Correct Answer:', style: TextStyle(fontWeight: FontWeight.bold, color: textColor)),
                    DropdownButton<String>(
                      value: currentAnswer,
                      dropdownColor: isDark ? const Color(0xFF1E1E24) : Colors.white,
                      style: TextStyle(color: textColor),
                      items: (isTF ? ['A', 'B'] : ['A', 'B', 'C', 'D']).map((ans) {
                        final label = isTF ? (ans == 'A' ? 'A (True)' : 'B (False)') : ans;
                        return DropdownMenuItem(value: ans, child: Text(label));
                      }).toList(),
                      onChanged: (val) {
                        if (val != null) setDialogState(() => currentAnswer = val);
                      },
                    ),
                  ],
                ),
              ...
              actions: [
                ...
                ElevatedButton(
                  style: ElevatedButton.styleFrom(backgroundColor: isDark ? Colors.yellow : Colors.blue),
                  onPressed: () async {
                    ...
                    try {
                      CheckMateUi.showTopPrompt(context, "Saving...", isError: false);
                      ...
                      if (mounted) {
                        ...
                        CheckMateUi.showTopPrompt(context, "Question updated!", isError: false);
                      }
                    } catch (e) {
                      if (mounted) CheckMateUi.showTopPrompt(context, "Failed to update: $e");
                    }
                  },
                ...
                children: [
                  ...
                  Expanded(
                    child: questions.isEmpty
                        ? const Center(child: Text("No questions found for this assessment."))
                        : StatefulBuilder(
                                    ...
                                    child: Padding(
                                      ...
                                      child: Column(
                                        ...
                                        children: [
                                            ...
                                            children: [
                                              ...
                                              if (widget.isOwner)
                                                IconButton(
                                                  ...
                                                  onPressed: () {
                                                    _showEditQuestionDialog(q, (updatedQ) {
                                                      setModalState(() {
                                          ...
                                          const SizedBox(height: 8),
                                          if (isTF) ...[
                                            const Text("A. True", style: TextStyle(color: Colors.grey)),
                                            const Text("B. False", style: TextStyle(color: Colors.grey)),
                                          ] else ...[
                                            ...List.generate(options.length, (i) {
                                              return Text("${String.fromCharCode(65 + i)}. ${options[i]}", style: const TextStyle(color: Colors.grey));
                                            }),
                                          ],
                                          const SizedBox(height: 8),
                                          Text(
  ...
  Future<void> _exportQuestionnaire(String examId, String title, {bool hasMultipleSets = false}) async {
    try {
      CheckMateUi.showTopPrompt(context, "Exporting questionnaire to DOCX...", isError: false);
      final questions = await SupabaseService.getExamQuestions(examId);
      if (questions.isEmpty) {
        if (mounted) CheckMateUi.showTopPrompt(context, "No questions found for this assessment.");
        return;
      }
      ...
      if (hasMultipleSets) {
        ...
        // Export Set A
        final bytesSetA = await ApiService.exportToDocx("$baseTitle - Set A", questions);
        // Export Set B
        final bytesSetB = await ApiService.exportToDocx("$baseTitle - Set B", setBQuestions);

        final fileNameSetA = "CheckMate_${baseTitle.replaceAll(' ', '_')}_SetA_${DateTime.now().millisecondsSinceEpoch}.docx";
        final fileSetA = File('${saveDir?.path ?? ""}/$fileNameSetA');
        await fileSetA.writeAsBytes(bytesSetA);

        final fileNameSetB = "CheckMate_${baseTitle.replaceAll(' ', '_')}_SetB_${DateTime.now().millisecondsSinceEpoch}.docx";
        ...
        final fileName = "CheckMate_${title.replaceAll(' ', '_')}_${DateTime.now().millisecondsSinceEpoch}.docx";
    ...
    if (confirm == true) {
      ...
      try {
        ...
        if (mounted) {
          ...
          CheckMateUi.showTopPrompt(context, "Assessment deleted successfully.", isError: false);
        }
      } catch (e) {
        if (mounted) {
      ...
      body: RefreshIndicator(
        ...
        child: FutureBuilder<List<Map<String, dynamic>>>(
          ...
          builder: (context, snapshot) {
            ...
            if (exams == null && snapshot.connectionState == ConnectionState.waiting) {
            ...
            return ListView(
              ...
              padding: const EdgeInsets.all(16),
              children: [
                _buildSection(context, "Quizzes", visibleExams.where((e) => (e['title'] ?? '').contains('[Quiz]')).toList()),
                const SizedBox(height: 24),
                _buildSection(context, "Exams", visibleExams.where((e) => (e['title'] ?? '').contains('[Exam]')).toList()),
                const SizedBox(height: 24),
                _buildSection(context, "Other Assessments", visibleExams.where((e) => !(e['title'] ?? '').contains('[Quiz]') && !(e['title'] ?? '').contains('[Exam]')).toList()),
              ],
            );
          },
        ),
    ...
    return Column(
      ...
      children: [
        ...
        if (isExpanded) ...items.map((exam) {
          ...
          displayTitle = displayTitle.replaceAll('[Quiz]', '').replaceAll('[Exam]', '').trim();
              ...
              child: Column(
                children: [
                  ListTile(
                    ...
                    title: Text(displayTitle, style: TextStyle(fontWeight: FontWeight.bold, color: textColor)),
                    subtitle: Text('Status: $status', style: TextStyle(color: textColor.withValues(alpha: 0.7))),
                    trailing: isApproved 
                      ...
                      child: Row(
                        ...
                        children: [
                          Row(
                            children: [
                              IconButton(
                                onPressed: _deletingExamId == exam['id'] ? null : () => _deleteExam(exam['id']),
                              ...
                              IconButton(
                                ...
                                onPressed: () => _exportQuestionnaire(
                                  exam['id'], 
                                  displayTitle, 
                                  hasMultipleSets: exam['has_multiple_sets'] == true,
                                ),
                          ...
                          Row(
                            children: [
                              ...
                              if (!isApproved)
                                ...
                                ElevatedButton.icon(
                                  onPressed: () async {
                                    await ApiService.releaseResults(exam['id']);
                                    setState(() {});
                                  },
                  ...
                  ] else if (!widget.isOwner && exam['results_released'] == true) ...[
                    const Divider(),
                    Padding(
                      padding: const EdgeInsets.symmetric(horizontal: 16),
                      child: Row(
