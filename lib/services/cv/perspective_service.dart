...
class PerspectiveService {
  ...
  static cv.Mat warpPaper(cv.Mat src, cv.VecPoint corners, double aspectRatio, int targetWidth, {double padding = 0.0}) {
    ...
    final double widthBottom = math.sqrt(math.pow(p3.x - p4.x, 2) + math.pow(p3.y - p4.y, 2));
    final double widthTop = math.sqrt(math.pow(p2.x - p1.x, 2) + math.pow(p2.y - p1.y, 2));
    final double maxWidth = math.max(widthBottom, widthTop);

    final double heightRight = math.sqrt(math.pow(p2.y - p3.y, 2) + math.pow(p2.x - p3.x, 2));
