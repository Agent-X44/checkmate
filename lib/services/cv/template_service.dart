...
class TemplateService {
  ...
  static List<Rect> detectColumnBoxes(cv.Mat thresholded) {
    ...
    // 1. Find Contours
    final (contours, _) =
        cv.findContours(thresholded, cv.RETR_EXTERNAL, cv.CHAIN_APPROX_SIMPLE);
  ...
  static cv.Mat extractAnswerRegion(cv.Mat warped, BubbleSheetTemplate template,
      {double xOffset = 0.0}) {
    return extractRegion(warped, template.answerRegions.first,
        xOffset: xOffset);
  }
  ...
  static List<cv.Mat> splitQuestions(
    ...
    for (int i = 5; i < h - 5; i++) {
      if (smoothed[i] > threshold &&
        ...
        if (peakIndices.isEmpty ||
            (i - peakIndices.last) > (h / (questionsInThisRegion * 1.5))) {
          peakIndices.add(i);
        }
      }
    }
    ...
    if (peakIndices.length >= questionsInThisRegion) {
      ...
      for (int i = 0; i < questionsInThisRegion; i++) {
        final int centerY =
            ((i + 0.5) * rowH).toInt() + yOffset + (i * ySpace).toInt();
        final int startY = math.max(0,
            math.min(h - 1, centerY - ((rowH * heightMultiplier) / 2).toInt()));
        final int actualH =
